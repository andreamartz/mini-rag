...
graph.addConditionalEdges('selectAgent', (state) => {
	return state.selectedAgent === 'linkedin' ? 'linkedInAgent' : 'ragAgent';
});
...
async function handleChat(query: string) {
	...
	if (selection.agent === 'linkedin') {
		...
		if (score < 6) {
			// Retry?
			return await linkedInAgent({
				query: selection.refinedQuery + ' (revised)',
			});
		}
...
const workflow = new StateGraph(schema)
	...
	.addConditionalEdges('validateQuery', (s) =>
		s.isValid ? 'selectAgent' : END
	)
	.addEdge('selectAgent', 'checkConfidence')
	.addConditionalEdges('checkConfidence', routeByConfidence)
	.addConditionalEdges('evaluateQuality', (s) =>
		s.score < 6 ? 'linkedInAgent' : END
	)
	.compile();
```
